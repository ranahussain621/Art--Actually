...
exports.session = async (req, res) => {
  try { 
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.LocalClienURL}thankyou/${userId}`,
      cancel_url: 'https://colorlib.com/wp/wp-content/uploads/sites/2/404-error-page-templates.jpg.webp',
...
// fetching customer subscription
exports.customerSub = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, error: 'Failed to list customer subscriptions' });
  }
};

// Is customer subscription expire or running
exports.checkSubscriptionStatus = async (req, res) => {
  try {
    ...
    if (subscriptions.data.length === 0) {
      return res.json({ success: true, status: 'No active subscription' });
    }

    const subscription = subscriptions.data[0];

    if (subscription.status === 'active' && subscription.current_period_end > Date.now()) {
      return res.json({ success: true, status: 'Active' });
    } else if (subscription.status === 'active' && subscription.current_period_end < Date.now()) {
...
exports.fetchCustomerList = async (req, res) => {
  try {
    ...
    return res.json({ success: true, customers: customers.data });
  } catch (error) {
...
exports.cancelClientSubscription = async (req, res) => {
  try {
      const user = await UserModel.find({ email: 'khizar.ijaz1997@gmail.com' });
      ...
      if (!user) {
      ...
      } else {
          {
              ...
              // var link = "http://localhost:4000/lawyer/accept-request/" + user[0]._id;
              // console.log(link);
              var mailOptions = {
